...
- **ContractRegistry expuesto a la WebUI (P1.4)**: nuevo artefacto `WebUI/src/contracts/gen/contractRegistry.gen.js` generado desde `modelContracts.gen.js` (sin Zod). Expone `contractRegistryData` con `mode`, `issues`, `hardwareLinks`, `importAdapters`, `exportAdapters`, `registeredModelIds`, `modelMetadata`. Re-exportado desde `WebUI/src/contracts/modelContracts.js`. Script `Scripts/build_contractRegistry_web.js` añadido a `npm run generate`.
