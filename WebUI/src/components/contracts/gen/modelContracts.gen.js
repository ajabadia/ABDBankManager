...
var behringerDm12Contract = {
  ...
  compatibleModels: ["behringer-deepmind6", "behringer-deepmind12d"],
...
var behringerDm6Contract = {
  ...
  parsePatchSysEx(sysex) {
    ...
    return { rawData: unpack7to83(sysex.slice(10, 10 + PACKED_SIZE2)).slice(0, PATCH_DATA_SIZE4), slot };
  },
  buildDumpRequest(slot, _channel) {
  ...
  parseDumpResponse(sysex) {
    return splitSysex5(sysex).flatMap((message) => {
      const parsed = this.parsePatchSysEx?.(message);
      return parsed ? [parsed] : [];
    });
  },
...
var behringerDm12dContract = {
  ...
  parsePatchSysEx(sysex) {
    ...
    return { rawData: unpack7to84(sysex.slice(10, 10 + PACKED_SIZE3)).slice(0, PATCH_DATA_SIZE5), slot };
  },
  buildDumpRequest(slot, _channel) {
  ...
  parseDumpResponse(sysex) {
    return splitSysex6(sysex).flatMap((message) => {
      const parsed = this.parsePatchSysEx?.(message);
      return parsed ? [parsed] : [];
    });
  },
...
var CATEGORIES7 = ["Bass", "Lead", "Pad", "FX", "Keys", "Perc", "Synth", "Other"];
...
var behringerPro800Contract = {
  ...
  parsePatchSysEx(sysex) {
    ...
    const packed = sysex.slice(11, sysex.length - 1);
    const unpacked = unpack7to85(packed);
...
var yamahaDx7Contract = {
  ...
  buildBulkSysEx(patches, channel) {
    const ch = channel - 1 & 15;
    const header = new Uint8Array([240, 67, ch, CMD_BULK, SUB_SINGLE, 0]);
...
var yamahaDx7iiContract = {
  ...
  legacySysEx: {
    modelIdByte: 1,
    buildDumpRequest: (ch) => new Uint8Array([240, 67, ch - 1 & 15, 1, CMD_BULK, SUB_SINGLE, 0, 247]),
