/**
 * ABD Bank Manager â€” Model Registry
 *
...
import { MODEL_CONTRACTS}
 ...
 from '../contracts/modelContracts.js';
import { decodePro800Parameters}

 from './pro800Parameters.js';
import { decodeDeepMindParameters}

 from './deepMindParameters.js';
import { decodeDx7Parameters, getDx7TableParameters}

 from './dx7Parameters.js';

// â”€â”€â”€ Parameter schema registry (built from contracts, no hardcodes) â”€â”€â”€
...
for (const contract of MODEL_CONTRACTS) {
  ...
  if (key && SCHEMA_DECODE_MAP[key]) {
    const { decode, getTable}

 = SCHEMA_DECODE_MAP[key];
    PARAMETER_SCHEMAS[contract.modelId] = {
      decode,
      getTable,
      formatLabel: rawData => `${contract.displayName}
 Â· ${contract.patchDataSize}
 bytes`,
   }
...
// â”€â”€â”€ MIDI detection patterns (built from contracts, no hardcodes) â”€â”€â”€
...
// â”€â”€â”€ Cached lookups (built once at load) â”€â”€â”€

/** @type {Map<string, object>}
 modelId â†’ contract */
const contractMap = new Map(MODEL_CONTRACTS.map(c => [c.modelId, c]));

/** @type {Array<{id: string, name: string, manufacturer: string}
>}
 sorted model list */
 ...
 * @param {string}
 modelId
 * @returns {{ decode: Function, getTable: Function, formatLabel: Function}
 ...
 * @param {string}
 portName
 * @returns {{ modelId: string, displayName: string}
...
export function detectModelFromPortName(portName) {
  ...
  for (const { pattern, modelId, displayName}

 of MIDI_DETECT_PATTERNS) {
 ...
 * @param {string}
 modelId
 * @returns {string}
 URL path (thumbnail or placeholder)
 */
export function getModelThumbnail(modelId) {
  const thumb = contractMap.get(modelId)?.thumbnail;
  return thumb ? `./vendor/images/models/thumbs/${thumb}
` : './vendor/images/models/thumbs/placeholder-synth.svg';
}
/**
