...
interface ModelContract {
  ...
  programsPerBank: number;      // Patches por banco (e.g., 16, 64, 16, 128)
  ...
  // SOLO se permite copiar patches entre modelos que aparezcan en esta lista.
  // El UI debe bloquear drag & drop y menú "Copy to Bank" entre modelos no compatibles.
...
interface PatchData {
  ...
  rawData: Uint8Array;          // Blob binario del patch en formato nativo
  parameters?: Record<string, number>; // Parámetros decodificados (opcional, para previsualización)
  isFavorite: boolean;
...
interface Patch {
  ...
  originModel: string;          // ModelContract.modelId del que proviene
  ...
  parameters: Record<string, number> | null;  // Parámetros decodificados
...
interface Bank {
  id: string;                   // UUID
  name: string;
  modelId: string;              // A qué sintetizador pertenece
  isFactory: boolean;           // Read-only si es factory
  isLocked: boolean;            // Protección contra edición accidental
  patches: Patch[];
...
5. El Bank Manager del plugin valida que `manifest.modelId === 'korg-ms2000'` y lo acepta.
...
El sistema **impide** transferir patches entre hardwares no compatibles. Solo se permite copiar/mover entre modelos que declaren compatibilidad mutua en `compatibleModels[]`.
