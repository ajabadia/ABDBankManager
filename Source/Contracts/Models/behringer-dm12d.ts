/**
 * ABD Universal Bank Manager — Behringer DeepMind 12D ModelContract
 *
 * The DeepMind 12D is the desktop module version of the DeepMind 12.
 * It shares the exact same SysEx format:
 * - Same manufacturer ID: 00 20 32
 * - Same model ID byte: 0x20
 * - Same 7-to-8 packing
 * - Same 242-byte patch data size
 *
 * Differences from DM12:
 * - Desktop module form factor (no keyboard, no pitch/mod wheels)
 * - Same 12 voices
 * - Same patch compatibility
 */
...
// Reuse DM12 constants — same SysEx format
const PATCH_DATA_SIZE = 242;
const PATCH_NAME_MAX_LENGTH = 16;
const MODEL_ID = 0x20;
...
const CATEGORIES = ['Bass', 'Lead', 'Pad', 'FX', 'Keys', 'Perc', 'Synth', 'UNK'];
...
const behringerDm12dContract: ModelContract = {
  ...
  parameterSchemaKey: 'behringer-deepmind12', // Same parameters as DM12
  ...
  buildPatchSysEx(rawData: Uint8Array, slot: number, _channel: number): Uint8Array {
    const data = new Uint8Array(PATCH_DATA_SIZE);
    data.set(rawData.slice(0, PATCH_DATA_SIZE));
  ...
  parsePatchSysEx(sysex: Uint8Array): { rawData: Uint8Array; slot: number } | null {
    ...
    return { rawData: unpack7to8Dm(sysex.slice(10, 10 + PACKED_SIZE)).slice(0, PATCH_DATA_SIZE), slot };
  },
  ...
  detectHardware(ports: Array<{ name?: string; id?: string }>): { name: string; inputId: string; outputId: string; manufacturer: string; modelId: string } | null {
    ...
    return port
      ? { name: port.name || 'DeepMind 12D', inputId: port.id || '', outputId: port.id || '', manufacturer: 'Behringer', modelId: this.modelId }
      : null;
  },
  legacySysEx: {
    modelIdByte: MODEL_ID,
    buildDumpRequest: () => new Uint8Array([0xF0, ...MANUFACTURER_ID, MODEL_ID, DEVICE_ID, CMD_REQUEST, 0x00, 0x00, 0xF7]),
    validateSysEx: bytes => isDeepMindMessage(bytes)
  }
};

export const allBehringerDm12dContracts = [behringerDm12dContract];
...
export default behringerDm12dContract;
