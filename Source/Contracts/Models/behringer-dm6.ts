/**
 * ABD Universal Bank Manager — Behringer DeepMind 6 ModelContract
 *
 * The DeepMind 6 shares the same SysEx format as the DeepMind 12:
 ...
 * - 6 voices instead of 12
 * - Desktop module form factor (no keyboard)
...
const PROTOCOL_VERSION = 0x06; // DM6 uses protocol version 6
const CMD_DUMP = 0x02;
...
const behringerDm6Contract: ModelContract = {
  ...
  detectHardware(ports: Array<{ name?: string; id?: string }>): { name: string; inputId: string; outputId: string; manufacturer: string; modelId: string } | null {
    ...
    return port
      ? { name: port.name || 'DeepMind 6', inputId: port.id || '', outputId: port.id || '', manufacturer: 'Behringer', modelId: this.modelId }
      : null;
  },
  legacySysEx: {
...
export const allBehringerDm6Contracts = [behringerDm6Contract];
...
export default behringerDm6Contract;
